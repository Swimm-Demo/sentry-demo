---
title: Understanding handled_snuba_exceptions
---
# Overview of handled_snuba_exceptions

The `handled_snuba_exceptions` flow is a sequence of function calls that handle exceptions that occur during interactions with Snuba, a column-oriented, distributed data warehouse that Sentry uses for its event processing. The flow starts with the `handled_snuba_exceptions` function, which is designed to catch and handle any exceptions that occur during Snuba queries.
...
The `get_event_stats_data` function is called within the `get` function. It takes several parameters including a request, an organization, a `get_event_stats` function, and others. It prepares the necessary parameters and calls the `get_event_stats` function. It then serializes the result and returns it.
...
The `get_event_stats` function is passed as a parameter to the `get_event_stats_data` function. It takes several parameters including columns, a query, parameters, a rollup, a boolean indicating whether to zerofill results, and a comparison delta. It prepares the necessary parameters and performs a series of queries to get event stats.
...
subgraph src/sentry/api/endpoints
  handled_snuba_exceptions:::mainFlowStyle --> fn:::mainFlowStyle
end
subgraph src/sentry/api/endpoints
...
The `fn` function is a complex function that performs various operations related to event statistics. It first checks if metrics are enhanced and if a dashboard widget ID is provided. If not, it calls the `_get_event_stats` function. If these conditions are met, it retrieves the dashboard widget and checks if it has a split and if an override feature is enabled. Depending on these conditions, it modifies the query and dataset and calls the `_get_event_stats` function again. It also handles errors and makes decisions based on the presence of errors and other data.
...
The `_get_event_stats` function is responsible for getting event statistics. It checks if there are top events. If there are, it calls the `top_events_timeseries` function of the scoped dataset. If there are no top events, it calls the `timeseries_query` function of the scoped dataset. It also handles various configurations and parameters related to metrics.
...
The `top_events_timeseries` function performs timeseries queries for a limited number of top events. It first checks if top events are provided. If not, it performs a query to get the top events. It then creates a `TopSpansMetricsQueryBuilder` and runs the query. If the number of top events equals the limit and other events are to be included, it creates another `TopSpansMetricsQueryBuilder` for other events and runs the query. It then processes the results, adds the order to the results, and returns the results.
...
The `timeseries_query` function performs arbitrary user timeseries queries against events. It creates a `TimeseriesSpansMetricsQueryBuilder`, runs the query, processes the results, and returns the results. It also handles various configurations and parameters related to metrics.
