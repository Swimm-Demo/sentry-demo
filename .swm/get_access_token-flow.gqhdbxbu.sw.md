---
title: get_access_token flow
---
<SwmSnippet path="/src/sentry/integrations/jira_server/client.py" line="258">

---

# get_access_token

The `get_access_token` function is the third step in the OAuth flow. It uses the verifier and request token from the first step to get an access token. If the verifier is not provided, it raises an ApiError. It then constructs an OAuth1 object with the necessary parameters and makes a POST request to the access token URL. The response is parsed and returned as a dictionary.
...
The `handle_assignee_change` function checks if the assignee field has changed in the data received. If it hasn't, the function returns. If the assignee field has changed, it checks if the assignee is None. If it is, it calls the `sync_group_assignee_inbound` function with assign set to False. If the assignee is not None, it gets the assignee email and calls the `sync_group_assignee_inbound` function with assign set to True.
...
The `assign` function is used to assign a user to a group. It first gets the selected options from the request. It then updates the group with the assignee and logs an info message. It also records an analytics event for the assignment. Finally, it sends a message indicating that the assignee has been updated.
...
The `send_message` function is used to send a new follow-up message. It checks if the message is a string and if so, it constructs a DiscordMessageBuilder object. It then returns a Response object with the message data.
...
The `remove_old_assignees` function is also called within the `deassign` function. It removes the old assignees from the group subscription. If the organization has the 'team-workflow-notifications' feature, it deletes the group subscription for the team that was previously assigned. If the new assignee is a user who is a member of the team that was previously assigned, they are not removed.
...
The `invalidate_assignee_exists_cache` function is used to clear the assignee exists cache. If a group ID is provided, the cache for that group is cleared. Otherwise, the cache for all groups in a project that had an event within the `ASSIGNEE_EXISTS_DURATION` window is cleared.
...
subgraph src/sentry/integrations/jira_server
  get_access_token:::mainFlowStyle --> post:::mainFlowStyle
end
subgraph src/sentry/integrations/jira_server
...
subgraph src/sentry/models
  invalidate_assignee_exists_cache:::mainFlowStyle --> cp5fs[...]
end
